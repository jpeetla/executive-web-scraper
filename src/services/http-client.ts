...
export class HttpClient {
  ...
  private async executeWithRetry<T>(
    ...
    Logger.warn(
      `Request failed after ${this.config.retries + 1} attempts: ${
        lastError?.message
      }`
    );
    return {
  ...
  async get<T = string>(url: string): Promise<HttpResponse<T>> {
    ...
    if (response.status >= 400) {
      Logger.warn(
        `Failed to fetch URL: ${url} with status code ${response.status}`
      );
    }
  ...
  async checkRobotsTxt(baseUrl: string): Promise<boolean> {
    try {
      ...
      const userAgentSection = lines.findIndex(
        (line) =>
      ...
      Logger.warn(
        `Failed to fetch robots.txt from ${baseUrl}: ${
          (error as Error).message
        }`
      );
