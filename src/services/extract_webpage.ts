import { COMMON_EXECUTIVE_KEYWORDS, KEYWORDS } from "../config/constants";
...
export async function cleanContentforLLM(
  content: string,
  url: string
): Promise<string> {
...
function extractTopTwoSections(content: string): string {
  ...
  if (combinedSections.length > MAX_CHARS) {
    ...
    if (cutoff > MAX_CHARS * 0.8) {
      // Only truncate at sentence if we keep at least 80%
...
export async function getLinkedinURLs(
  ...
  if (executives) {
    for (const executive of executives) {
      ...
      const linkedinUrl =
        linkedin_serp_results.find((url) => url.includes("linkedin.com/in")) ||
        "";
      ...
      const isDuplicate = executivesData.some(
        (existingExecutive) =>
          existingExecutive.linkedin === executiveObject.linkedin
      );
      if (!isDuplicate) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      pageContent = pageContent
        ...
        .replace(/\s+/g, " ")
        .replace(
          /\b(class|id|style|div|span|width|height|margin|padding|color|font|text-align|href|src|alt|meta|css|html|doctype|javascript)\b/g,
          ""
        )
        .trim();
